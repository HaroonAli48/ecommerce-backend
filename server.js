...
function getDateTime() {
  const now = new Date();
  const formatted = now.toISOString().replace(/[-:.TZ]/g, '').slice(0, 14);
  return formatted;
}
...
app.get('/pay', (req, res) => {
  ...
  const formAction = 'https://sandbox.jazzcash.com.pk/CustomerPortal/transactionmanagement/merchantform/';

  let form = `<form id="jazzForm" method="post" action="${formAction}">`;
  for (let key in payload) {
    form += `<input type="hidden" name="${key}" value="${payload[key]}" />`;
  }
  form += '</form><script>document.getElementById("jazzForm").submit();</script>';
...
app.post('/payment/response', (req, res) => {
  ...
  if (responseData.pp_ResponseCode === '000') {
    ...
    res.send('❌ Payment Failed. Response Code: ' + responseData.pp_ResponseCode);
  }
});
